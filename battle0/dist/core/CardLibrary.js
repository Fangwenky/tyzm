import { NormalAttackCard, ActionCard, EnduringGuardCard, LethalStingCard, DefensiveStanceCard, DesperateBlowCard, FireAttackCard, SwiftAttackCard, HeavyPunchCard, DoubleTapCard, LeechingStrikeCard, MightyBlowCard, SacredProtectionCard, CharcoalCard, VenomCard, CollapseCard, WaterMirrorCard, LifeGrowthCard, TenaciousGuardCard, AllOutAttackCard, RapidPursuitCard, ArmorlessPursuitCard, SpecialActionCard, StrengthFromHealthCard, BuffNextCardAction, 
// 新增卡牌类
CriticalStrikeCard, FollowUpAttackCard, WeaponMasteryCard, ShadowCopyCard, AttackAsDefenseCard, StrengthBoostCard, BerserkerAssaultCard, VitalStrikeCard, HeadlongStrikeCounter, DamageAbsorptionCounter, NightmareGazeCounter, HarmoniousResonanceCounter, ArmorProtectionEquipment, UniversalProtectionEquipment, PursuitBladeEquipment, HeavyStrikeHammerEquipment, BloodthirstyBladeEquipment, DualNatureEquipment, BattleAxeEquipment, RocketFireworksEquipment, RocketModificationCard, 
// 监管者装备类
...
var cardDataPool = {
    ...
    'e011': { id: 'e011', name: '重重一击', type: 'Action', description: '对敌方造成12点伤害', cost: 1, damage: 12, targetType: 'enemy', image: 'assets/images/cards/mighty_blow.png' },
    'e012': { id: 'e012', name: '力量压制', type: 'Action', description: '对敌方造成自己当前生命1/5的伤害(最多10)', cost: 1, targetType: 'enemy', image: 'assets/images/cards/strength_boost.png' },
    // --- 厂长专属 ---
    'e101': { id: 'e101', name: '木炭', type: 'Normal', description: '造成10点火焰伤害', damage: 10, targetType: 'enemy', image: 'assets/images/cards/charcoal.png' },
...
var CardLibrary = /** @class */ (function () {
    function CardLibrary() {
        ...
        this.playerCardIds = Object.keys(this.allCards).filter(function (id) { return id.startsWith('p'); });
        this.enemyCommonCardIds = Object.keys(this.allCards).filter(function (id) { return id.startsWith('e0'); });
        this.enemyExclusiveCardIds = {
            '厂长': Object.keys(this.allCards).filter(function (id) { return id.startsWith('e1'); }),
            '小丑': Object.keys(this.allCards).filter(function (id) { return id.startsWith('e2'); }),
            '蜘蛛': Object.keys(this.allCards).filter(function (id) { return id.startsWith('e3'); }),
            '愚人金': Object.keys(this.allCards).filter(function (id) { return id.startsWith('e4'); }),
            '红夫人': Object.keys(this.allCards).filter(function (id) { return id.startsWith('e5'); }),
            '噩梦': Object.keys(this.allCards).filter(function (id) { return id.startsWith('e6'); }),
        };
    }
    CardLibrary.prototype.getCardData = function (cardId) {
        return this.allCards[cardId];
    };
    CardLibrary.prototype.getPlayerCardsData = function () {
        var _this = this;
        return this.playerCardIds.map(function (id) { return _this.allCards[id]; });
    };
    CardLibrary.prototype.createCard = function (cardId) {
        var data = this.getCardData(cardId);
        if (!data)
            throw new Error("\u5728\u5361\u724C\u6C60\u4E2D\u627E\u4E0D\u5230ID\u4E3A ".concat(cardId, " \u7684\u5361\u724C"));
        switch (data.id) {
            // 原有卡牌
            ...
            default:
                ...
                if (data.type === 'Counter')
                    return new HeadlongStrikeCounter(data); // 默认反制卡
                if (data.type === 'Equipment')
                    return new ArmorProtectionEquipment(data); // 默认装备卡
                if (data.cost)
                    return new ActionCard(data);
                return new NormalAttackCard(data);
        }
    };
    CardLibrary.prototype.buildRandomEnemyDeck = function (enemyName, deckSize, exclusiveCount) {
        var deck = [];// 敌方牌组
        var exclusivePool = this.enemyExclusiveCardIds[enemyName] || [] // 敌方专属卡牌
        //从监管者通用卡池中选取10张 e001，4张 e003，2 张 e010，1 张 e011，1 张e012 加入敌方牌组
        deck.push(this.createCard('e001'));
        deck.push(this.createCard('e001'));
        deck.push(this.createCard('e001'));
        deck.push(this.createCard('e001'));
        deck.push(this.createCard('e001'));
        ...
        deck.push(this.createCard('e001'));
        deck.push(this.createCard('e001'));
        deck.push(this.createCard('e003'));
        deck.push(this.createCard('e003'));
        deck.push(this.createCard('e003'));
        deck.push(this.createCard('e003'));
        deck.push(this.createCard('e010'));
        deck.push(this.createCard('e010'));
        deck.push(this.createCard('e011'));
        deck.push(this.createCard('e012'));
        //从监管者专属卡牌中每张获取两个加入敌方牌组
        exclusivePool.forEach(function (cardId) {
...
export { CardLibrary };
//# sourceMappingURL=CardLibrary.js.map