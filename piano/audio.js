...
(function() {
    ...
    // 键盘按键与音符的映射：1-7对应从中央C开始的7个连续音符
// 数值代表与中央C的半音差（0 = 中央C，依次递增为C#、D、D#、E、F、F#、G...）
const keyToNoteMap = {
  '1': 0-12,   // 1键 -> 中央C（C4）
  '2': 2-12,   // 2键 -> D4（比C高2个半音）
  '3': 4-12,   // 3键 -> E4
  '4': 5-12,   // 4键 -> F4
  '5': 7-12,   // 5键 -> G4
  '6': 9-12,   // 6键 -> A4（标准音440Hz）
  '7': 11-12   // 7键 -> B4
};

// 监听键盘按下事件
document.addEventListener('keydown', function(e) {
  const key = e.key; // 获取按下的键
  const note = keyToNoteMap[key]; // 查找对应的音符

  if (note !== undefined) {
    e.preventDefault(); // 阻止按键默认行为（如滚动页面）
    // 播放对应音符，可自定义音频配置（波形、时长等）
    const sound = Notes.getCachedSound(note, {
      styleFn: DataGenerator.style.wave, // 正弦波（柔和音色）
      volume: 20000, // 音量（0-32767）
      seconds: 0.5   // 每个音符时长0.5秒
    });
    sound.play(); // 播放音符
  }
